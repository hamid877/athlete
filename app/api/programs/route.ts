...
import type { IProgramTemplateDay, IWorkoutTemplateExercise } from "@/models/ProgramTemplate";
...
export async function POST(request: Request) {
  try {
    ...
    const { name, splitType, templateId } = parsed.data;
    ...
    try {
      ...
      if (templateId) {
        ...
        const { default: Exercise } = await import("@/models/exercise.model");

        const template = await ProgramTemplate.findById(templateId).session(sessionDB);
        if (!template) {
        ...
        template.workoutDays.forEach((wd: IProgramTemplateDay) => {
          if (wd.workout && !wd.isRestDay) {
            wd.workout.exercises.forEach((ex: IWorkoutTemplateExercise) => {
              slugs.add(ex.exerciseSlug);
            });
          }
        });

        // Fetch exercises and build slug-to-id map
        const exercises = await Exercise.find({ slug: { $in: Array.from(slugs) } }).session(sessionDB);
        const slugToId = new Map(exercises.map(ex => [ex.slug, ex._id]));

        // Check for missing slugs
        const missingSlugs = Array.from(slugs).filter(slug => !slugToId.has(slug));
        if (missingSlugs.length > 0) {
          await sessionDB.abortTransaction();
          sessionDB.endSession();
          return NextResponse.json(
            { error: `Missing exercises in database: ${missingSlugs.join(", ")}` },
            { status: 400 }
          );
        }

        for (const wd of template.workoutDays) {
          if (wd.isRestDay || !wd.workout) {
            workoutDays.push({ day: wd.day as DayOfWeek, workoutId: null, isRestDay: true });
            continue;
          }

          const workoutExercises = wd.workout.exercises.map((ex: IWorkoutTemplateExercise) => ({
            exerciseId: slugToId.get(ex.exerciseSlug),
            order: ex.order,
            sets: ex.sets,
            repRange: { min: ex.repRange.min, max: ex.repRange.max },
            rest: ex.rest,
          }));
        ...
        const nameSequence = WORKOUT_NAMES_BY_SPLIT[splitType as string] ?? [];
        let nameIndex = 0;
